...
use tricciardi\UsenditSms\Soap\Request\SendMessages;
...
class UsenditSms
{
    ...
    public function send($to, $message)
    {
        $this->soapWrapper->add('SendMessages', function ($service) {
            $service
                ->wsdl('https://api.usendit.co.mz/v2/remoteusendit.asmx?WSDL')
                ->trace(true)->classmap([
                    SendMessages::class,
        ...
        $response = $this->soapWrapper->call('SendMessages.SendMessages', [
            new SendMessages(config('usenditsms.username'), config('usenditsms.password'), null, [$sms])
        ]);
    ...
    public function sendBatch($items)
    {
        ...
        $response = $this->soapWrapper->call('SendMessages.SendMessages', [
            new SendMessages(config('usenditsms.username'), config('usenditsms.password'), null, $smsList)
        ]);
        return $response;
    }
}
